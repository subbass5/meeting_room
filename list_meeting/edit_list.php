	...
	<body>
        ...
        $crud = new CRUD();
		    if(isset($_REQUEST['id'])){
		        ...
		        $strQury = "SELECT * FROM meeting WHERE meeting_id =".$id.";";
	         	$res = $crud->ElseCondition($strQury);
			      if(mysql_num_rows($res)>0)
			        {
				           echo '  <form action="list_meeting/save_list.php" method="POST">
				                   <input type="hidden" name="filename" value="manage_room">
				                   <table class="table">';
				          while($obj = mysql_fetch_assoc($res))
				           { 
					
                   echo '    <input type="hidden" name="idmeeting" value="'.$obj['meeting_id'].'">
                              <td><p><label>ชื่อหัวข้อ</label></p>
                                 <p><input type="text" name="title" class="form-control" value="'.$obj['title'].'" required></p>
                              </td>
                         ...
                         <tr>
                              <td><p><label>เวลาเริ่มต้น</label></p>
                                 <p><input type="datetime" name="timestart" id="testdate5" class="form-control" value="'.$obj['date_time_start'].'" required></p>
                              </td>
                         </tr>

                         <tr>
                              <td><p><label>เวลาสิ้นสุด</label></p>
                                 <p><input type="datetime" name="timeend" id="testdate6" class="form-control" value="'.$obj['date_time_end'].'" required/></p>
                              </td>
                         </tr>';?>
                         <tr>
                              <td><p>ห้องประชุม</p>
