...
<body >
 ...
 <center><h4>ตารางการใช้ห้องประชุมวันนี้ <br />
  ( <?php	$eng_date1 = date("Y-m-d") ; 
  														// แสดงวันที่ปัจจุบัน
...
?>
	<?php 
		$res1=mysql_query("SELECT meeting.title,meeting.date_time_start,meeting.date_time_end,users.name FROM meeting,users WHERE meeting.user_id = users.user_id and meeting.room_id = $id and meeting.state = 1 and meeting.date_time_start like '%$date%' 
...
<a href="report.php?date=<?php echo $date ?>" target="_blank">  <span  class="glyphicon glyphicon-print" aria-hidden="true">   </span> <br />
